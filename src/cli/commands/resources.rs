...
impl ResourcesCommand {
    async fn handle_json_entity(
        ...
        let final_resource_name: String = exposed_resources
            .resources
            .clone()
            .into_iter()
        ...
        let res = Otoroshi::otoroshi_call(
            hyper::Method::POST,
            format!("/apis/any/v1/{}/{}", final_resource_name, id).as_str(),
            None,
        ...
        type: \"object\"\n",
            ...
            ResourcesSubCommand::Template { resource, kube } => {
                ...
                let exposed_resource = exposed_resources
                    ...
                    .unwrap();
                match Otoroshi::get_resource_template(
                    exposed_resource.clone().plural_name,
                    cli_opts.clone(),
                )
                .await
                {
                                    ...
                                    match cli_opts.ouput {
                                        Some(str) => match str.as_str() {
                                            ...
                                            "json_pretty" => cli_stdout_printline!(
                                                "{}",
                                                serde_json::to_string_pretty(&Self::with_kind(
                                                    &raw_resource.body,
                                                    res_kind
                                                ))
                                                .unwrap()
                                            ...
                                            "yaml" => {
                                                if kube.unwrap_or(true) {
                                                    ...
                                                    let res_name =
                                                        EntityHelper::extract_json_entity_name(
                                                            &res_body,
                                                        )
                                                        .unwrap();
                                                    let kube_res = KubeEntity::new(
                                                    ...
                                                    cli_stdout_printline!(
                                                        "{}",
                                                        serde_yaml::to_string(&Self::with_kind(
                                                            &raw_resource.body,
                                                            res_kind
                                                        ))
                                                        .unwrap()
                            ...
                            .await
                            {
                                Some(raw_resources) => {
                                    match cli_opts.ouput {
                                        Some(str) => match str.as_str() {
                                            "json" => {
                                                let items: Vec<serde_json::Value> = raw_resources
                                                    .body
                                                    .into_iter()
                                                    .map(|v| {
                                                        Self::with_kind(&v, res_kind.to_string())
                                                    })
                                                    .collect();
                                                cli_stdout_printline!(
                                                    "{}",
                            ...
                            if split_files.unwrap_or(false) {
                                ...
                                for entity in res.body {
                                        ...
                                        && !kube.unwrap_or(true)
                                    {
                                        ...
                                        kind.insert(
                                            "kind".to_string(),
                                            serde_json::Value::String(resource.clone().kind),
                                        );
