...
impl VersionCommand {
    ...
    fn default_display(version: OtoroshiVersion) -> () {
        ...
        .table()
        .title(vec![
            "version".cell().bold(true).foreground_color(Some(cli_table::Color::Green)),
        ...
        match Otoroshi::get_version(cli_opts.clone()).await {
            ...
            Some(version) => {
                match cli_opts.ouput {
                    Some(str) => {
                        match str.as_str() {
                            "json" => cli_stdout_printline!("{}", serde_json::to_string(&version).unwrap()),
                            "json_pretty" => cli_stdout_printline!("{}", serde_json::to_string_pretty(&version).unwrap()),
                            "yaml" => cli_stdout_printline!("{}", serde_yaml::to_string(&version).unwrap()),
