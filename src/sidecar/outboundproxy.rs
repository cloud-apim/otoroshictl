...
use crate::sidecar::config::{OtoroshiSidecarConfigSpecOtoroshiSettingsRoutingLocation, OtoroshiSidecarConfigSpecOtoroshiSettingsLocationKubernetesLocation};

use super::config::OtoroshiSidecarConfig;
use super::cache::{SidecarCache, OtoroshiCertificate};
...
impl OutboundProxyClient {
    ...
    fn new(config: OtoroshiSidecarConfig, cli_opts: CliOpts) -> OutboundProxyClient {
        ...
        OutboundProxyClient {
            ...
            config,
            tls_client,
            mtls_clients: CacheBuilder::new(50).time_to_live(std::time::Duration::from_secs(120)).build(),
        }
    }

    fn mtls_client(&self, id: String, client_cert: OtoroshiCertificate) -> Client<HttpsConnector<hyper::client::HttpConnector>> {
    ...
    fn request(&self, mut req: Request<Body>, client_cert: Option<(String, OtoroshiCertificate)>) -> ResponseFuture {
        let otoroshi_spec = self.config.clone().spec.compute_otoroshi(self.cli_opts.clone());
        let otoroshi_routing_location = otoroshi_spec.routing_location.unwrap_or(OtoroshiSidecarConfigSpecOtoroshiSettingsRoutingLocation::default());
        let otoroshi_routing_location_clone = otoroshi_routing_location.clone();
        let uri_string = if self.config.clone().spec.kubernetes {
            let kube = otoroshi_routing_location_clone.kubernetes.unwrap_or(OtoroshiSidecarConfigSpecOtoroshiSettingsLocationKubernetesLocation::default());
            format!(
                "{}://{}:{}{}",
                if otoroshi_routing_location.tls { "https" } else { "http" },
...
impl OutboundProxy {
    pub fn start(cli_opts: CliOpts, config: OtoroshiSidecarConfig, cache: Arc<SidecarCache>) -> impl Future<Output = std::result::Result<(), hyper::Error>> {
        let in_addr: SocketAddr = SocketAddr::new("127.0.0.1".parse().unwrap(), config.spec.outbounds.port.unwrap_or(15001));
...
impl Svc {
    fn get_additional_headers(&self, host: String) -> HashMap<http::HeaderName, http::HeaderValue> {
        match self.config.spec.outbounds.outbounds.get(&host) {
            Some(outbound) => {
                match outbound.clone().apikey {
                    Some(apikey_config) if apikey_config.enabled => {
                        match self.cache.get_apikey_by_id(apikey_config.apikey_id.clone()) {
                            Some(apikey) => {
                                let mut headers = HashMap::new();
                                headers.insert(http::HeaderName::from_static("otoroshi-client-id"), http::HeaderValue::from_str(apikey.clientId.as_str()).unwrap());
                                headers.insert(http::HeaderName::from_static("otoroshi-client-secret"), http::HeaderValue::from_str(apikey.clientSecret.as_str()).unwrap());
                                headers
                            },
                            _ => {
                                error!("apikey with id '{}' does not exists", apikey_config.apikey_id.clone());
                                HashMap::new()
                            }
                        }
                    },
...
impl Service<Request<Body>> for Svc {
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        let host = headers.get("Host").or(headers.get(":authority")).or(Some(&authority_value)).unwrap_or(&default_value);
        let host_str = host.to_str().unwrap().to_string();
        if host_str.ends_with(self.config.spec.dns_domain.clone().unwrap_or(".otoroshi.mesh".to_string()).as_str()) {
