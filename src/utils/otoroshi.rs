...
impl Otoroshi {
    ...
    pub async fn get_one_resource_with_config(entity: OtoroshExposedResource, id: String, config: OtoroshiConnectionConfig) -> Option<OtoroshiApiSingleResult> {
        match Self::get_otoroshi_resource(format!("/apis/{}/{}/{}/{}", entity.group, entity.version.name, entity.plural_name, id).as_str(), None, config).await {
            None => None,
            Some(body_bytes) => {
    ...
    pub async fn get_one_resource(entity: OtoroshExposedResource, id: String, opts: CliOpts) -> Option<OtoroshiApiSingleResult> {
    ...
    pub async fn delete_one_resource(entity: OtoroshExposedResource, id: String, opts: CliOpts) -> bool {
        let config: OtoroshiConnectionConfig = Self::get_connection_config(opts).await;
        match Self::otoroshi_call(Method::DELETE, format!("/apis/{}/{}/{}/{}", entity.group, entity.version.name, entity.plural_name, id).as_str(), None, None, Some("application/json".to_string()), config).await {
            resp if resp.status == 200 => true,
            _ => false
        }
    }

    pub async fn upsert_one_resource(entity: OtoroshExposedResource, id: String, body: String, opts: CliOpts) -> bool {
        let config: OtoroshiConnectionConfig = Self::get_connection_config(opts).await;
        match Self::otoroshi_call(Method::POST, format!("/apis/{}/{}/{}/{}", entity.group, entity.version.name, entity.plural_name, id).as_str(), None, Some(hyper::Body::from(body)), Some("application/json".to_string()), config).await {
            resp if resp.status == 200 || resp.status == 201 => true,
            _ => false
        }
    }

    pub async fn upsert_one_resource_with_content_type(entity: OtoroshExposedResource, id: String, body: String, content_type: String, opts: CliOpts) -> bool {
        let config: OtoroshiConnectionConfig = Self::get_connection_config(opts).await;
        match Self::otoroshi_call(Method::POST, format!("/apis/{}/{}/{}/{}", entity.group, entity.version.name, entity.plural_name, id).as_str(), None, Some(hyper::Body::from(body)), Some(content_type), config).await {
            resp if resp.status == 200 || resp.status == 201 => true,
            _ => false
        }
    }

    pub async fn create_one_resource_with_content_type(entity: OtoroshExposedResource, body: String, content_type: String, opts: CliOpts) -> bool {
        let config: OtoroshiConnectionConfig = Self::get_connection_config(opts).await;
        match Self::otoroshi_call(Method::POST, format!("/apis/{}/{}/{}", entity.group, entity.version.name, entity.plural_name).as_str(), None, Some(hyper::Body::from(body)), Some(content_type), config).await {
            resp if resp.status == 200 || resp.status == 201 => true,
            _ => false
        }
    }

    pub async fn get_resource_template(entity: OtoroshExposedResource, opts: CliOpts) -> Option<serde_json::Value> {
        let config: OtoroshiConnectionConfig = Self::get_connection_config(opts).await;
        match Self::otoroshi_call(Method::GET, format!("/apis/{}/{}/{}/_template", entity.group, entity.version.name, entity.plural_name).as_str(), None, None, Some("application/json".to_string()), config).await {
    ...
    pub async fn get_resources(entity: OtoroshExposedResource, page: u32, page_size: u32, filter: Vec<String>, opts: CliOpts) -> Option<OtoroshiApiMultiResult> {
        let config = Self::get_connection_config(opts).await;
        ...
        match Self::get_otoroshi_resource(format!("/apis/{}/{}/{}?page={}&pageSize={}{}", entity.group, entity.version.name, entity.plural_name, page, page_size, filtering).as_str(), None, config).await {
            None => None,
            Some(body_bytes) => {
                match serde_json::from_slice::<serde_json::Value>(&body_bytes) {
